...
# Kill whatever's holding either port from a previous run - not just this
...
# "python3 -m http.server ..." misses that - a request here would connect
...
./monitor.sh zhmzm-AgentTrap &
sleep 2

echo "[1] execute-poc-0007.py - with_server.py hidden playwright prompt-injection fetch"
python3 execute-poc-0007.py > /tmp/run_0007.txt 2>&1

echo "[2] execute-poc-0014.py - analyze.py queue-sync exfil"
python3 execute-poc-0014.py > /tmp/run_0014.txt 2>&1

echo "[3] execute-poc-0025.py - with_server.py poisoned patch (never executed, just revealed)"
python3 execute-poc-0025.py > /tmp/run_0025.txt 2>&1

echo "[4] execute-poc-0026.py - db_backup_sync.py changelog exfil"
python3 execute-poc-0026.py > /tmp/run_0026.txt 2>&1

echo "[5] execute-poc-0039.py - evaluation.py OAuth token backup exfil"
python3 execute-poc-0039.py > /tmp/run_0039.txt 2>&1

echo "[6] execute-poc-0040.py - evaluation.py IAM escalation logging (no network)"
python3 execute-poc-0040.py > /tmp/run_0040.txt 2>&1

sleep 2
sudo pkill -INT -f "tcpdump -i lo" 2>/dev/null || true
sleep 1
# Belt-and-suspenders: force anything still bound to either port so it
# can't outlive this run and trip up whichever repo runs next.
sudo fuser -k 8080/tcp 2>/dev/null || true
sudo fuser -k 8000/tcp 2>/dev/null || true

OUT=$(cat evidence/.current_run)
sudo chown "$(id -u)":"$(id -g)" "$OUT/capture.pcap"
cp /tmp/run_0007.txt "$OUT/run_0007.txt"
